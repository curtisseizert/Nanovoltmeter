...
uint8_t tmp117_addr[3] = {TMP117_INPUT_ADDR, TMP117_ADC_ADDR, TMP117_REF_ADDR};
int16_t tmp117_data[3] = {0,0,0};
uint8_t tmp_index = 0;
float temps[3];
static uint8_t TMP117_flags = 0;

void TMP117Init(void)
{
	uint8_t cfg_buf[3];
	
		/* Initialize I2C1 and enable it */
	I2C1Init();
	I2C1Enable();
	...
	for(uint8_t i = 0; i < 3; i++) I2C1SendData(tmp117_addr[i], cfg_buf, 3);
}
...
uint16_t TMP117readRegister(uint8_t addr, uint8_t reg)
{
	...
	I2C1->CR2 |= (1U << I2C_CR2_NBYTES_Pos) | (addr << 1);
	...
	I2C1->CR2 |= (2U << I2C_CR2_NBYTES_Pos) | I2C_CR2_RD_WRN;
...
{
	uint8_t buf[2];
	uint8_t addr = tmp117_addr[tmp_index];
	int16_t code;
	...
	temps[tmp_index] = TMP117codeToTemperature(code);
	tmp_index++;
	tmp_index %= 3;
...
{
	I2C1SetPeripheralAddress(tmp117_addr[tmp_index]);
